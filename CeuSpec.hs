...
main = hspec $ do
  ...
  describe "nst1" $ do
    ...
    describe "(Block p)" $ do
      it "transit: Block Nop" $
        ...
        `shouldBe` Just (Seq Nop (Envs' 1), 0, Nothing, [newEnv,newEnv])
    ...
    describe "(Write id exp)" $ do
      ...
      it "transit: [x=?] x=1" $
        ...
        `shouldBe` Just (Nop, 0, Nothing, [(["x"],[("x",1)])])

      it "transit: [x=1] x=2" $
        nst1 (Write "x" (Const 2), 0, Nothing, [(["x"],[("x",1)])])
        `shouldBe` Just (Nop, 0, Nothing, [(["x"],[("x",2),("x",1)])])
      ...
      it "nothing: [x=1] y=x+2" $
        nst1 (Write "y" (Add (Read "x") (Const 2)), 0, Nothing, [(["x"],[("x",1)])])
        `shouldBe` Nothing

      it "transit: [x=1,y=?] y=x+2" $
        nst1 (Write "y" (Add (Read "x") (Const 2)), 0, Nothing, [(["x","y"],[("x",1)])])
        `shouldBe` Just (Nop, 0, Nothing, [(["x","y"],[("y",3),("x",1)])])

      it "transit: [x=?] x=-(5+1)" $
        nst1 (Write "x" (Umn (Add (Const 5) (Const 1))), 0, Nothing, [(["x"],[])])
        `shouldBe` Just (Nop, 0, Nothing, [(["x"], [("x",-6)])])

    -- emit-int --
    describe "(EmitInt e')" $ do
      it "transit: lvl == 0" $
        nst1 (EmitInt 0, 0, Nothing, [([],[])])
        `shouldBe` Just (CanRun 0, 0, Just 0, [([],[])])

      it "transit: lvl > 0" $
        nst1 (EmitInt 1, 3, Nothing, [([],[])])
        `shouldBe` Just (CanRun 3, 3, Just 1, [([],[])])

      it "nothing: evt /= nil" $
        nst1 (EmitInt 1, 3, Just 1, [([],[])])
        `shouldBe` Nothing
    ...
    describe "(CanRun n)" $ do
      ...
      it "transit: n == lvl" $
        ...
        `shouldBe` Just (Nop, 8, Nothing, [([],[])])

      it "nothing: n > lvl" $
        nst1 (CanRun 8, 6, Nothing, [([],[])])
        `shouldBe` Nothing

      it "nothing: n < lvl" $
        nst1 (CanRun 8, 12, Nothing, [([],[])])
        `shouldBe` Nothing

      it "nothing: evt /= nil" $
        nst1 (CanRun 0, 0, Just 1, [([],[])])
        `shouldBe` Nothing
    ...
    describe "(Seq p q)" $ do
      ...
      it "nothing: evt /= nil" $
        nst1 (Seq (Seq Nop Nop) Nop, 0, Just 1, [([],[])])
        `shouldBe` Nothing

      it "nothing: isBlocked p" $
        nst1 (Seq (Fin Nop) Nop, 0, Nothing, [([],[])])
        `shouldBe` Nothing
    ...
    describe "(If exp p q)" $ do
      ...
      it "transit: x == 0" $
        ...
        `shouldBe` Just (Break, 0, Nothing, [(["x"],[("x",0)])])
    ...
    describe "(Loop p)" $ do
      it "transit: lvl == 0" $
        ...
        `shouldBe` Just (Seq (Loop' Nop Nop) (Envs' 1),0,Nothing,[([],[])])
      ...
      it "transit: isBlocked p" $
        ...
        `shouldBe` Just (Seq (Loop' (Fin Nop) (Fin Nop)) (Envs' 1),0,Nothing,[([],[])])
    ...
    describe "(Loop' p q)" $ do
      ...
      it "nothing: evt /= nil" $
        nst1 (Loop' (Seq Nop Nop) Nop, 0, Just 1, [([],[])])
        `shouldBe` Nothing

      it "nothing: isBlocked p" $
        nst1 (Loop' (Fin Nop) Nop, 0, Nothing, [([],[])])
        `shouldBe` Nothing
    ...
    describe "(And' p Break)" $ do
      ...
      it "nothing: evt /= nil" $
        nst1 (And' (Fin Nop) Break, 0, Just 1, [([],[])])
        `shouldBe` Nothing
    ...
    describe "(Or' p Break)" $ do
      ...
      it "nothing: evt /= nil" $
        nst1 (Or' (Fin Nop) Break, 0, Just 1, [([],[])])
        `shouldBe` Nothing
  ...
  describe "nst" $ do
    describe "Zero steps: Program is blocked" $ do
      ...
      it "CanRun -> CanRun#" $
        nst (CanRun 5, 0, Nothing, [([],[])])
        `shouldBe` (CanRun 5, 0, Nothing, [([],[])])
  ...
  describe "out1" $ do
    ...
    describe "pop" $ do
      ...
      it "transit: lvl > 0 && nst-irreducible" $
        out1 (Nop, 33, Nothing, [([],[])])
        `shouldBe` Just (Nop, 32, Nothing, [([],[])])

      it "nothing: lvl > 0 && not nst-irreducible" $
        out1 (Seq Nop Nop, 1, Nothing, [([],[])])
        `shouldBe` Nothing
