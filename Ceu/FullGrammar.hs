...
data Stmt
  ...
  | AwaitExt G.Evt (Maybe G.ID)   -- await external event
  | AwaitFor                      -- await forever
-- TODO: AwaitTmr
  | AwaitInt G.Evt (Maybe G.ID)   -- await internal event
  | EmitInt G.Evt G.Expr          -- emit internal event
  | Break                         -- loop escape
  | If G.Expr Stmt Stmt           -- conditional
  | Seq Stmt Stmt                 -- sequence
  | Loop Stmt                     -- infinite loop
  | Every G.Evt (Maybe G.ID) Stmt -- event iteration
...
remAsync p = (rA False p) where
  ...
  rA True  (Loop p)       = Loop (rA True (Seq p (AwaitExt inputAsync Nothing)))
...
remAwaitFor AwaitFor       = AwaitExt G.inputForever Nothing
remAwaitFor p              = p
...
toGrammar p = toG $ remAwaitFor $ remAsync
  ...
  toG (AwaitExt e id) = G.AwaitExt e id
  toG (AwaitInt e id) = G.AwaitInt e id
  toG (EmitInt e exp) = G.EmitInt e exp
  toG Break           = G.Break
  toG (If exp p1 p2)  = G.If exp (toG p1) (toG p2)
  toG (Seq p1 p2)     = G.Seq (toG p1) (toG p2)
  toG (Loop p)        = G.Loop (toG p)
  toG (Every e id p)  = G.Every e id (toG p)
