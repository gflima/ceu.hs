...
check (Seq p1 p2)  = (check p1) ++ x ++ (check p2)
  where
    x = if (infinite p1 || (escapes p1 /= [])) then [("unreachable statement",p2)] else []
...
infinite :: Stmt -> Bool
infinite (Var _ p)      = infinite p
infinite (Int _ p)      = infinite p
infinite (AwaitExt ext) = ext == "FOREVER"
infinite (If _ p1 p2)   = infinite p1 && infinite p2
infinite (Seq p1 p2)    = infinite p1 || infinite p2
infinite (Loop p)       = escapes p == []
infinite (Every _ p)    = True
infinite (Par p1 p2)    = infinite p1 || infinite p2
infinite (Pause _ p)    = infinite p
infinite (Fin p)        = True
infinite (Trap p)       = infinite p
infinite _              = False
