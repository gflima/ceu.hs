module Ceu.Grammar.NType where
...
data Type_ = TAny                         -- bot/sup
           | TUnit
           | TVar   ID_Var
           | TData  ID_Data_Hier [Type_]  -- X.Y of (Int,Bool) // data X.Y of (a,b) with (a,b)
           | TTuple [Type_]               -- (len >= 2)
           | TFunc  FuncType Type_ Type_  -- inp out
  deriving (Eq,Show)
...
type FT_Ups = (FuncType, Maybe (S.Set (ID_Var,Int)))
...
toString (_tp,cs) =
  case cs of
    [] -> _toString _tp
    l  -> _toString _tp ++ " where (" ++ L.intercalate "," (map f l) ++ ")" where
            f (var,[cls]) = var ++ " is " ++ cls
            f (var,clss)  = var ++ " is (" ++ L.intercalate "," clss ++ ")"

_toString :: Type_ -> String
_toString TAny            = "?"
_toString TUnit           = "()"
_toString (TData ids [])  = L.intercalate "." ids
_toString (TData ids [x]) = L.intercalate "." ids ++ " of " ++ _toString x
_toString (TData ids ofs) = L.intercalate "." ids ++ " of " ++ "(" ++ L.intercalate "," (map _toString ofs) ++ ")"
