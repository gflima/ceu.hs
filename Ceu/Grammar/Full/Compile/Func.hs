...
stmt (SClass z cls ctrs ifc) =
  case Cs.toList ctrs of
    [(var,_)]  -> SClass z cls ctrs (stmt $ aux ifc) where
      ...
      aux (SFunc z id env (tp_,ctrs) imp) = SFunc z id env (tp_, Cs.insert (var,cls) ctrs) imp
      aux p                           = p
...
stmt (SInst  z cls tp@(_,ctrs) imp)    = SInst  z cls tp (stmt $ aux imp)
  where
    ...
    aux (SFunc z id env (tp_',ctrs') imp) = SFunc z id env (tp_',Cs.union ctrs ctrs') imp
    aux p                             = p

stmt (SFunc z k env tp@(tp_,ctrs) imp) = SSeq z (SVar z k tp) (SSet z False (EVar z k) (EFunc z env tp (stmt imp')))
 where
