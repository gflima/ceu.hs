...
aux (Int z int b)                = Int' z int b (Nop z)
aux (Out z ext b)                = Out' z ext b (Nop z)
aux (If z exp p1 p2)             = If z exp (aux p1) (aux p2)
aux (Seq _ s@(Var z var fin) p2) = Var' z var (aux_fin fin) (aux p2)
aux (Seq _ s@(Int z int b) p2)   = Int' z int b (aux p2)
aux (Seq _ s@(Out z ext b) p2)   = Out' z ext b (aux p2)
