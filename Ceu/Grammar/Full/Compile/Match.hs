...
stmt (Set    z chk pat exp)   = stmt $ Match z chk exp [(Nop z,pat,Nop z)]
stmt (Match  z chk exp cses)  = Match' z chk  (expr exp) (map (\(ds,pt,st) -> (stmt ds, expr pt, stmt st)) cses')
                                where
                                  cses' = cses
...
stmt (If     z exp p1 p2)     = stmt $ Match z False exp [
                                  (Nop z, EExp z (EVar z "_true"), stmt p1),
                                  (Nop z, EAny z,                  stmt p2)
                                ]
