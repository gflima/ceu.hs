...
aux (AwaitExt ext (Just var)) = (AwaitExt ext Nothing) `Seq` (Write var (newExp$Read ("_"++ext)))
aux (AwaitInt int (Just var)) = (AwaitInt int Nothing) `Seq` (Write var (newExp$Read ("_"++int)))
...
aux (Every evt (Just var) p)  = Every evt Nothing
                                     ((Write var (newExp$Read ("_"++evt))) `Seq` (aux p))
