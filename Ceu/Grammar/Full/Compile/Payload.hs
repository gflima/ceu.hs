...
aux (AwaitExt ext (Just var)) = (AwaitExt ext Nothing) `Seq` (Write var (Exp$Read ("_"++ext)))
aux (AwaitInt int (Just var)) = (AwaitInt int Nothing) `Seq` (Write var (Exp$Read ("_"++int)))
...
aux (Every evt (Just var) p)  = Every evt Nothing
                                     ((Write var (Exp$Read ("_"++evt))) `Seq` (aux p))
