...
spec = do
  ...
  describe "simplify" $ do
    ...
    it "If a a -> a" $ do
      simplify (If (Exp$Const 1) (AwaitExt "") (AwaitExt "")) `shouldBe` (AwaitExt "")
    it "If x y -> If x y" $ do
      simplify (If (Exp$Const 1) (Escape 0) Nop) `shouldBe` (If (Exp$Const 1) (Escape 0) Nop)
    ...
    it "Fin -> Nop" $ do
      simplify (Fin (Var "" (If (Exp$Const 1) Nop (Escape 0)))) `shouldBe` (Fin (Var "" (If (Exp$Const 1) Nop (Escape 0))))
