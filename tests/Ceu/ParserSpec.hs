    ...
    describe "type:" $ do
        ...
        describe "typeN" $ do
            ...
            it "(Int,Int)" $
                ...
                `shouldBe` Right (TypeN [TypeD ["Int"], TypeD ["Int"]])
            it "(Int,())" $
                parse type_N "(Int,())"
                `shouldBe` Right (TypeN [TypeD ["Int"], Type0])
        ...
        describe "pType" $ do
            ...
            it "(Int, ((),()))" $
                parse pType "(Int, ((),()))"
                `shouldBe` Right (TypeN [TypeD ["Int"], TypeN [Type0,Type0]])
    ...
    describe "stmt:" $ do
        ...
        describe "var:" $ do
            it "var x: Int" $
                ...
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)}))) (Nop (annz{source = ("",1,1)})))
            it "var val x" $
                parse stmt_var "var val x"
                `shouldBe` Left "(line 1, column 9):\nunexpected \"x\"\nexpecting \":\""
            it "var a: Int <- 1" $
                parse stmt_var "var a : Int <- 1"
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "a" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)}))) (Set (annz{source = ("",1,13)}) False (LVar "a") (Number (annz{source = ("",1,16)}) 1)))
            it "var x:() <- ()" $
            ...
            it "var x:(Int,()) <- (1 ())" $
                ...
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeN [TypeD ["Int"],Type0],cz)) (Nop (annz{source = ("",0,0)}))) (Set (annz{source = ("",1,16)}) False (LVar "x") (Tuple (annz{source = ("",1,19)}) [Number (annz{source = ("",1,20)}) 1,Unit (annz{source = ("",1,22)})])))
        ...
        describe "var-tuples:" $ do
            ...
            it "var (x,y) : (Int,Int) <- (1, 2); return x+y" $
                ...
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeD ["Int"],cz)) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "y" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)})))) (Set (annz{source = ("",1,23)}) False (LTuple [LVar "x",LVar "y"]) (Tuple (annz{source = ("",1,26)}) [Number (annz{source = ("",1,27)}) 1,Number (annz{source = ("",1,30)}) 2])))
            it "var (x,(y,_)):(Int,(Int,Int)) <- (1, (2,3)); return x+y" $
                parse stmt "var (x,(y,_)):(Int, (Int,Int)) <- (1, (2,3)); return x+y"
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeD ["Int"],cz)) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "y" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)})))) (Set (annz{source = ("",1,32)}) False (LTuple [LVar "x",LTuple [LVar "y",LAny]]) (Tuple (annz{source = ("",1,35)}) [Number (annz{source = ("",1,36)}) 1,Tuple (annz{source = ("",1,39)}) [Number (annz{source = ("",1,40)}) 2,Number (annz{source = ("",1,42)}) 3]]))) (Ret (annz{source = ("",1,47)}) (Call (annz{source = ("",1,55)}) (Read annz{source=("",1,55)} "+") (Tuple (annz{source = ("",1,54)}) [Read (annz{source = ("",1,54)}) "x",Read (annz{source = ("",1,56)}) "y"]))))
...
-------------------------------------------------------------------------------
        ...
        describe "func:" $ do
            it "var add : ..." $
                ...
                `shouldBe` Right (Seq annz{source = ("",1,1)} (Seq annz (Var annz{source = ("",1,1)} "add" (TypeF (TypeN [TypeD ["Int"],TypeD ["Int"]]) (TypeD ["Int"]),cz)) (Nop annz)) (Nop annz{source = ("",1,1)}))
            it "var add : ... <- func ..." $
                parse stmt "var add : ((Int, Int) -> Int) <- func (a,_) : ((Int, Int) -> Int) do end"
                `shouldBe` Right (Seq annz{source=("",1,1)} (Seq annz (Var annz{source=("",1,1)} "add" (TypeF (TypeN [TypeD ["Int"],TypeD ["Int"]]) (TypeD ["Int"]),cz)) (Nop annz)) (Set annz{source=("",1,31)} False (LVar "add") (Func annz{source=("",1,34)} (TypeF (TypeN [TypeD ["Int"],TypeD ["Int"]]) (TypeD ["Int"]),cz) (Seq annz{source=("",1,34)} (Seq annz (Var annz{source=("",1,34)} "a" (TypeD ["Int"],cz)) (Nop annz)) (Seq annz{source=("",1,34)} (Set annz{source=("",1,34)} False (LTuple [LVar "a",LAny]) (Arg annz{source=("",1,34)})) (Nop annz{source=("",1,70)}))))))
            it "func add : (...) do end" $
                parse stmt_funcs "func add (a,_) : ((Int, Int) -> Int) do end"
                `shouldBe` Right (FuncS annz{source=("",1,1)} "add" (TypeF (TypeN [TypeD ["Int"],TypeD ["Int"]]) (TypeD ["Int"]),cz) (Seq annz{source=("",1,1)} (Seq annz{source=("",0,0)} (Var annz{source=("",1,1)} "a" (TypeD ["Int"],cz)) (Nop annz{source=("",0,0)})) (Seq annz{source=("",1,1)} (Set annz{source=("",1,1)} False (LTuple [LVar "a",LAny]) (Arg annz{source=("",1,1)})) (Nop annz{source=("",1,41)}))))
            it "func add (...) : (...)" $
            ...
            it "func add : (...)" $
                ...
                `shouldBe` Right (Var annz{source=("",1,1)} "add" (TypeF (TypeN [TypeD ["Int"],TypeD ["Int"]]) (TypeD ["Int"]),cz))
            it "func (_,_,_) : (_,_)" $
            ...
            it "and" $
              ...
              `shouldBe` Right (Seq annz (FuncS annz "and" (TypeF (TypeN [TypeD ["Bool"],TypeD ["Bool"]]) (TypeD ["Bool"]),cz) (Seq annz (Seq annz (Var annz "x" (TypeD ["Bool"],cz)) (Seq annz (Var annz "y" (TypeD ["Bool"],cz)) (Nop annz))) (Seq annz (Set annz False (LTuple [LVar "x",LVar "y"]) (Arg annz)) (Ret annz (Cons annz ["Bool","False"] (Unit annz)))))) (Ret annz (Call annz (Read annz "and") (Tuple annz [Cons annz ["Bool","True"] (Unit annz),Cons annz ["Bool","True"] (Unit annz)]))))
        ...
        describe "data" $ do
            ...
            it "data Xxx ; var x = Xxx" $
              ...
              `shouldBe` Right (Seq annz (Data annz ["Xxx"] (Type0,cz) False) (Seq annz (Seq annz (Var annz "x" (TypeD ["Xxx"],cz)) (Nop annz)) (Set annz False (LVar "x") (Cons annz ["Xxx"] (Unit annz)))))
            it "data Xxx.Yyy" $
            ...
            it "data Xxx with (Int,Int)" $
              ...
              `shouldBe` Right (Data annz{source=("",1,1)} ["Xxx"] (TypeN [TypeD ["Int"],TypeD ["Int"]],cz) False)

            it "data Xxx with (Int)" $
              (parse' stmt "data Xxx with (Int)")
              `shouldBe` Right (Data annz ["Xxx"] (TypeD ["Int"],cz) False)

            it "data Xxx with Int ; x<-Xxx(1,1)" $
              (parse' stmt "data Xxx with Int ; var x:Xxx <- Xxx 1")
              `shouldBe` Right (Seq annz (Data annz ["Xxx"] (TypeD ["Int"],cz) False) (Seq annz (Seq annz (Var annz "x" (TypeD ["Xxx"],cz)) (Nop annz)) (Set annz False (LVar "x") (Cons annz ["Xxx"] (Number annz 1)))))
            ...
            it "Xxx.Yyy" $
              ...
              `shouldBe` ([],B.Data annz ["Int"] (Type0,cz) False (B.Data annz ["Xxx"] (TypeD ["Int"],cz) False (B.Data annz ["Xxx","Yyy"] (TypeN [TypeD ["Int"],TypeD ["Int"]],cz) False (B.Var annz "y" (TypeD ["Xxx","Yyy"],cz) (B.Match annz False (B.LVar "y") (B.Cons annz{type_=(TypeD ["Xxx","Yyy"],cz)} ["Xxx","Yyy"] (B.Tuple annz{type_=(TypeN [TypeD ["Int","1"],TypeD ["Int","2"]],cz)} [B.Number annz{type_=(TypeD ["Int","1"],cz)} 1,B.Number annz{type_=(TypeD ["Int","2"],cz)} 2])) (B.Nop annz) (B.Ret annz (B.Error annz (-2))))))))

            it "data X with Int ; x:Int ; X x <- X 1 ; ret x" $
              (parse' stmt "data Xxx with Int ; var x:Int ; set Xxx x <- Xxx 1 ; return x")
              `shouldBe` Right (Seq annz (Data annz ["Xxx"] (TypeD ["Int"],cz) False) (Seq annz (Seq annz (Seq annz (Var annz "x" (TypeD ["Int"],cz)) (Nop annz)) (Nop annz)) (Seq annz (Set annz False (LCons ["Xxx"] (LVar "x")) (Cons annz ["Xxx"] (Number annz 1))) (Ret annz (Read annz "x")))))
            it "data X with Int ; X 1 <- X 1 ; return 1" $
              (parse' stmt "data Xxx with Int ; set Xxx 1 <- Xxx 1 ; return 1")
              `shouldBe` Right (Seq annz (Data annz ["Xxx"] (TypeD ["Int"],cz) False) (Seq annz (Set annz False (LCons ["Xxx"] (LNumber 1)) (Cons annz ["Xxx"] (Number annz 1))) (Ret annz (Number annz 1))))
            it "data X with Int ; x:Int ; X 1 <- X 2" $
              (parse' stmt "data Xxx with Int ; set Xxx 1 <- Xxx 2 ; return 2")
              `shouldBe` Right (Seq annz (Data annz ["Xxx"] (TypeD ["Int"],cz) False) (Seq annz (Set annz False (LCons ["Xxx"] (LNumber 1)) (Cons annz ["Xxx"] (Number annz 2))) (Ret annz (Number annz 2))))
          ...
          it "Int ; IF3able a ; inst IF3able Int ; return f3 1" $
            ...
            `shouldBe` Right
              ...
              (Class annz{source=("",1,1)} "IF3able" (cv "a")
                (Seq annz{source=("",2,2)}
                (Seq annz{source=("",0,0)}
                (Var annz{source=("",2,2)} "f3" (TypeF (TypeV "a") (TypeD ["Int"]),cz))
                (Nop annz{source=("",0,0)}))
                (Nop annz{source=("",2,2)})))
              (Seq annz{source=("",1,1)}
              (Inst annz{source=("",4,1)} "IF3able" (TypeD ["Int"],cz)
                (FuncS annz{source=("",5,2)} "f3" (TypeF (TypeV "a") (TypeD ["Int"]),cz)
                  (Seq annz{source=("",5,2)}
                  (Seq annz{source=("",0,0)}
          ...
          it "IOrd extends IEq" $
            ...
            `shouldBe` Right
              ...
              (Class annz "IOrd" (cvc("a","IEq"))
                (Var annz ">=" (TypeF (TypeN [TypeV "a",TypeV "a"]) (TypeD ["Bool"]),cz)))
              (Seq annz
              (Inst annz "IEq" (TypeD ["Bool"],cz)
                (FuncS annz "==" (TypeF (TypeN [TypeV "a",TypeV "a"]) (TypeD ["Bool"]),cz)
                  (Seq annz (Seq annz (Var annz "x" (TypeV "a",cz)) (Seq annz (Var annz "y" (TypeV "a",cz)) (Nop annz))) (Seq annz (Set annz False (LTuple [LVar "x",LVar "y"]) (Arg annz)) (Ret annz (Cons annz ["Bool","True"] (Unit annz)))))))
              (Seq annz
              (Inst annz "IOrd" (TypeD ["Bool"],cz)
                (FuncS annz ">=" (TypeF (TypeN [TypeV "a",TypeV "a"]) (TypeD ["Bool"]),cz)
                  (Seq annz (Seq annz (Var annz "x" (TypeV "a",cz)) (Seq annz (Var annz "y" (TypeV "a",cz)) (Nop annz))) (Seq annz (Set annz False (LTuple [LVar "x",LVar "y"]) (Arg annz)) (Ret annz (Cons annz ["Bool","True"] (Unit annz)))))))
            ...
            `shouldBe` Right
              (Seq annz
                (Class annz "IFable" (cv "a")
                  (Var annz "f" (TypeF (TypeV "a") (TypeD ["Bool"]),cz)))
              (Seq annz
                (FuncS annz "g" (TypeF (TypeV "a") (TypeD ["Bool"]),cvc("a","IFable"))
                  (Seq annz
                  (Seq annz
        ...
        describe "seq:" $ do
            ...
            it "var a:Int ; a<-1" $
                ...
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "a" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)}))) (Nop (annz{source = ("",1,1)}))) (Set (annz{source = ("",1,18)}) False (LVar "a") (Number (annz{source = ("",1,20)}) 1)))

        describe "stmt:" $ do
            it "var x:Int; return 1" $
                parse stmt "var x:Int ;return 1"
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)}))) (Nop (annz{source = ("",1,1)}))) (Ret (annz{source = ("",1,12)}) (Number (annz{source = ("",1,19)}) 1)))

            it "var x:Int; x<-1; return x" $
                parse stmt "var x:Int ; set x <- 1 ; return x"
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeD ["Int"],cz)) (Nop (annz{source = ("",0,0)}))) (Nop (annz{source = ("",1,1)}))) (Seq (annz{source = ("",1,1)}) (Set (annz{source = ("",1,19)}) False (LVar "x") (Number (annz{source = ("",1,22)}) 1)) (Ret (annz{source = ("",1,26)}) (Read (annz{source = ("",1,33)}) "x"))))

            it "var x:(Int,Int,)<-(1,2) ; return +x" $
                parse stmt "var x:(Int,Int)<-(1,2) ; return +x"
                `shouldBe` Right (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",1,1)}) (Seq (annz{source = ("",0,0)}) (Var (annz{source = ("",1,1)}) "x" (TypeN [TypeD ["Int"],TypeD ["Int"]],cz)) (Nop (annz{source = ("",0,0)}))) (Set (annz{source = ("",1,16)}) False (LVar "x") (Tuple (annz{source = ("",1,18)}) [Number (annz{source = ("",1,19)}) 1,Number (annz{source = ("",1,21)}) 2]))) (Ret (annz{source = ("",1,26)}) (Call (annz{source = ("",1,33)}) (Read annz{source=("",1,33)} "+") (Read (annz{source = ("",1,34)}) "x"))))
