...
spec = do
  ...
  describe "typeclass" $ do
    ...
    it "A ; Xable.fff(a) ; Inst Xable (A,A) ; fff(A,A)" $
      ...
      `shouldBe` (["missing implementation of 'fff'"],Data annz ["A"] [] Type0 False (Var annz "fff" (TypeF (TypeV "a") Type0) (Var annz "_inst__Xable__(A,A)" (TypeN [TypeF (TypeN [Type1 ["A"],Type1 ["A"]]) Type0]) (Var annz "fff__((A,A) -> ())" (TypeF (TypeN [Type1 ["A"],Type1 ["A"]]) Type0) (Match annz False (LVar "_inst__Xable__(A,A)") (Tuple annz [Error annz (-2)]) (Match annz False (LTuple [LVar "fff__((A,A) -> ())"]) (Read annz "_inst__Xable__(A,A)") (CallS annz (Call annz{type_=Type0} (Read (annz {type_ = TypeF (TypeN [Type1 ["A"],Type1 ["A"]]) Type0}) "fff__((A,A) -> ())") (Tuple (annz {type_ = TypeN [Type1 ["A"],Type1 ["A"]]}) [Cons (annz {type_ = Type1 ["A"]}) ["A"] (Unit (annz {type_ = Type0})),Cons (annz {type_ = Type1 ["A"]}) ["A"] (Unit (annz {type_ = Type0}))]))) (Ret annz (Error annz (-2)))) (Ret annz (Error annz (-2))))))))
