...
spec = do
  ...
  describe "functions" $ do
    ...
    it "Int ; X a ; inst X Int ; return fff 1" $
      (fst $ TypeSys.go $ Simplify.go
        (Data annz ["Int"] [] (Type0,[]) False
        (Class annz "X" [("a",[])] [(annz,"fff",(TypeF (TypeV "a") (TypeD ["Int"]),[("a",["X"])]),False)]
            (Var annz "fff" (TypeF (TypeV "a") (TypeD ["Int"]),[("a",["X"])])
  ...
  describe "typeclass:" $ do
    ...
    it "X.f ; Y.f" $
      (fst $ TypeSys.go
        (Class annz "X" [("a",[])] []
        (Var annz "f" (TypeF (TypeV "a") Type0,[("a",["X"])])
        (Class annz "Y" [("a",[])] []
        (Var annz "f" (TypeF (TypeV "a") Type0,[("a",["Y"])])
        (Nop annz))))))
    ...
    it "Bool ; Equalable ; (==)" $
      TypeSys.go
        (Data annz ["Bool"] [] (Type0,[]) True
        (Class annz "Equalable" [("a",[])] []
        (Var annz "==" (TypeF (TypeN [(TypeV "a"),(TypeV "a")]) (TypeD ["Bool"]), [("a",["Equalable"])])
        (Nop annz))))
    ...
    it "A ; Xable.fff(a) ; Inst Xable (A,A) ; fff(A,A)" $
      TypeSys.go
        (Data annz ["A"] [] (Type0,[]) False
        (Class annz "Xable" [("a",[])] [(annz,"fff",(TypeF (TypeV "a") Type0,[]),False)]
        (Var annz "fff" (TypeF (TypeV "a") Type0, [("a",["Xable"])])
        ...
        (Match annz False (LVar "$fff$((A,A) -> ())$")
          ...
          (CallS annz
            (Call (annz {type_ = (Type0,[])})
              ...
              (Tuple
                (annz {type_ = (TypeN [TypeD ["A"],TypeD ["A"]],[])}) [Cons (annz {type_ = (TypeD ["A"],[])}) ["A"] (Unit (annz {type_ = (Type0,[])})),Cons (annz {type_ = (TypeD ["A"],[])}) ["A"] (Unit (annz {type_ = (Type0,[])}))])))
          (Ret annz (Error annz 99))))))
    ...
    it "Int ; A ; Xable a ; inst Xable A ; fff 1" $
      (fst $ TypeSys.go
        (Data annz ["Int"] [] (Type0,[]) False
        (Data annz ["A"] [] (Type0,[]) False
        (Class annz "Xable" [("a",[])] [(annz,"fff",(TypeF (TypeV "a") Type0, [("a",["Xable"])]),False)]
        (Var annz "fff" (TypeF (TypeV "a") Type0, [("a",["Xable"])])
        (Inst annz "Xable" (TypeD ["A"],[])
          [(annz, "fff", (TypeF (TypeD ["A"]) Type0,[]), True)]
          (func "$fff$(A -> ())$" (TypeF (TypeD ["A"]) Type0,[])
