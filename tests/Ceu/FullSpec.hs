...
-}
  ...
  describe "Scope.compile" $ do
    ...
    describe "var:" $ do
      ...
      it "var x <- 1 ; (Nop annz)" $ do
        Scope.compile (Seq annz (Var annz "x" (int,cz)) (Seq annz (Set annz False (LVar "x") (Number annz 1)) (Nop annz)))
        `shouldBe` (Var'' annz "x" (int,cz) (Seq annz (Set annz False (LVar "x") (Number annz 1)) (Nop annz)))
  ...
  describe "compile'" $ do
    ...
    it "if" $
      ...
      `shouldBe` Seq annz (Match' annz True (LExp (Read annz "_true")) (Call annz (Cons annz ["Bool","False"]) (Unit annz)) (Nop annz) (Nop annz)) (Ret annz (Number annz 10))
    ...
    it "class/inst/0" $ do
      ...
      `shouldBe`
        ...
        (Var'' annz "$f3$(Int -> Int)$" (TypeF (TypeV "Int") (int),cz)
          (Match' annz False (LVar "$f3$(Int -> Int)$") (Func annz (TypeF (TypeV "Int") (int),cz) (Ret annz (Number annz 10))) (Nop annz) (Ret annz (Error annz (-2))))))
    ...
    it "class/inst/2" $ do
      ...
      `shouldBe`
        ...
        (Inst'' annz "F3able" (int,cz)
          [(annz,"f3",(TypeF (TypeV "Int") (int),cz),True)]
        (Var'' annz "$f3$(Int -> Int)$" (TypeF (TypeV "Int") (int),cz)
            (Match' annz False (LVar "$f3$(Int -> Int)$") (Func annz (TypeF (TypeV "Int") (int),cz) (Ret annz (Number annz 10)))
              (Seq annz
                (Nop annz)
                (Ret annz (Call annz (Read annz "f3") (Number annz 10))))
    ...
    it "Xxx.Yyy" $
      compile (Seq annz (Data annz (int,                            M.fromList []) False)
              ...
              (Seq annz (Var annz "y" (TypeD ["Xxx","Yyy"] [] Type0,M.fromList []))
                        (Nop annz))
                        (Set annz False (LVar "y") (Call annz (Cons annz ["Xxx","Yyy"]) (Tuple annz [Number annz 1,Number annz 2])))))))
      `shouldBe`
        (Data'' annz (TypeD ["Int"] [] Type0,fromList []) False (Data'' annz (TypeD ["Xxx"] [] int,fromList []) False (Data'' annz (TypeD ["Xxx","Yyy"] [] (TypeN [int,int]),fromList []) False (Var'' annz "y" (TypeD ["Xxx","Yyy"] [] (TypeN [int,int]),fromList []) (Seq annz (Nop annz) (Match' annz False (LVar "y") (Call annz (Cons annz ["Xxx","Yyy"]) (Tuple annz [Number annz 1,Number annz 2])) (Nop annz) (Ret annz (Error annz (-2)))))))))
