...
{-
  describe "TODO" $ do
    it "TODO" $
      compile' (Seq annz (Data annz ["Int"] [] TUnit False) (Seq annz (Seq annz (Var annz "x" (TData ["Int"])) (Nop annz)) (Seq annz (Set annz False (LVar "x") (Cons annz ["Int","1"])) (Ret annz (Cons annz ["Int","1"])))))
...
-}
    ...
    it "func id : (a->a) do end" $ do
      ...
      `shouldBe` (Seq annz
                  ...
                  (Set annz False (LVar "id")
                    (Func annz (TFunc (TAny "a") (TAny "a"),cz) (Nop annz))))
  ...
  describe "Scope.compile" $ do
    ...
    describe "var:" $ do
      ...
      it "scope var x end ; x=1" $ do
        ...
        `shouldBe` (["data 'Int' is not declared","variable 'x' is not declared","data 'Int.1' is not declared"], B.Seq annz (B.Var annz "x" (int,cz) (B.Nop annz)) (B.Match annz False (B.LVar "x") (B.Cons (annz{type_=(TAny "?",cz)}) ["Int","1"]) (B.Nop annz) (B.Ret annz (B.Error annz (-2)))))
  ...
  describe "compile'" $ do
    ...
    it "do var x; x = 1 end" $ do
      ...
      `shouldBe` (["data 'Int' is not declared","data 'Int.1' is not declared"], (B.Var annz "x" (int,cz) (B.Match annz False (B.LVar "x") (B.Cons annz{type_=(TData ["Int"] [] TUnit,cz)} ["Int","1"]) (B.Nop annz) (B.Nop annz))))
    ...
    it "class/inst/0" $ do
      ...
      `shouldBe`
        ...
        (Var'' annz "$f3$(Int -> Int)$" (TFunc (TAny "Int") (int),cz)
          (Match' annz False (LVar "$f3$(Int -> Int)$") (Func annz (TFunc (TAny "Int") (int),cz) (Ret annz (Cons annz ["Int","10"]))) (Nop annz) (Ret annz (Error annz (-2))))))
    ...
    it "class/inst/2" $ do
      ...
      `shouldBe`
        ...
        (Inst'' annz "F3able" (int,cz)
          [(annz,"f3",(TFunc (TAny "Int") (int),cz),True)]
        (Var'' annz "$f3$(Int -> Int)$" (TFunc (TAny "Int") (int),cz)
            (Match' annz False (LVar "$f3$(Int -> Int)$") (Func annz (TFunc (TAny "Int") (int),cz) (Ret annz (Cons annz ["Int","10"])))
              (Seq annz
                (Nop annz)
                (Ret annz (Call annz (Read annz "f3") (Cons annz ["Int","10"]))))
    ...
    it "class/inst/3" $ do
      ...
      `shouldBe`
        ...
        (Inst'' annz "F3able" (int,cz)
          ...
          (Match' annz False
            (LVar "$f3$(Int -> Int)$")
            (Func annz (TFunc (TAny "Int") (int),cz)
              (Var'' annz "v" (TAny "Int",cz)
              (Seq annz
              (Nop annz)
    ...
    it "Xxx.Yyy" $
      ...
      `shouldBe`
        (Data'' annz (TData ["Int"] [] TUnit,fromList []) False (Data'' annz (TData ["Xxx"] [] int,fromList []) False (Data'' annz (TData ["Xxx","Yyy"] [] (TTuple [int,int]),fromList []) False (Var'' annz "y" (TData ["Xxx","Yyy"] [] (TTuple [int,int]),fromList []) (Seq annz (Nop annz) (Match' annz False (LVar "y") (Call annz (Cons annz ["Xxx","Yyy"]) (Tuple annz [Cons annz ["Int","1"],Cons annz ["Int","2"]])) (Nop annz) (Ret annz (Error annz (-2)))))))))
  ...
  describe "go" $ do
    ...
    it "data X with Int ; x:Int ; X 1 <- X 2" $ do
      go (prelude annz $ Seq annz (Data annz (TData ["Xxx"] [] int,cz) False) (Seq annz (Set annz False (LCons ["Xxx"] (LCons ["Int","1"] LUnit)) (Call annz (Cons annz ["Xxx"]) (Cons annz ["Int","2"]))) (Ret annz (Cons annz ["Int","2"]))))
      `shouldBe`
