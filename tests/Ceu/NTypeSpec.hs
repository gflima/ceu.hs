module Ceu.NTypeSpec (main, spec) where

import Debug.Trace
import Test.Hspec

import Ceu.Grammar.Globals
import Ceu.Grammar.NType
...
bool  = TData ["Bool"]         []
boolf = TData ["Bool","False"] []
boolt = TData ["Bool","True"]  []
...
-}
  ...
  describe "_toString" $ do
    it "()" $
      _toString TAny `shouldBe` "?"
    it "X.Y" $
      _toString (TData ["X","Y"] []) `shouldBe` "X.Y"
    it "X.Y of ()" $
      _toString (TData ["X","Y"] [TUnit]) `shouldBe` "X.Y of ()"
    it "X.Y of ((),())" $
      _toString (TData ["X","Y"] [TUnit,TUnit]) `shouldBe` "X.Y of ((),())"

  describe "toString" $ do
    it "()" $
      toString (TAny,cz) `shouldBe` "?"
    it "()" $
      toString (TAny,[("a",["IEqualable"])]) `shouldBe` "? where (a is IEqualable)"
