...
spec = do
  ...
  describe "remVar" $ do
    ...
    it "x = 1;" $ do
      remVar (Locals (Write "x" (G.Const 1)))
      `shouldBe` (Locals' [] (Write "x" (G.Const 1)))

    it "var x; x = 1" $ do
      remVar (Locals (Seq (Var "x") (Write "x" (G.Const 1))))
      `shouldBe` (Locals' ["x"] (Seq Nop' (Write "x" (G.Const 1))))

    it "var x || x = 1" $ do
      remVar (Locals (Or (Var "x") (Write "x" (G.Const 1))))
      `shouldBe` (Locals' ["x"] (Or Nop' (Write "x" (G.Const 1))))

    it "do var x; x = 1 end" $ do
      remVar (Locals (Locals (Seq (Var "x") (Write "x" (G.Const 1)))))
      `shouldBe` (Locals' [] (Locals' ["x"] (Seq Nop' (Write "x" (G.Const 1)))))
  ...
  describe "remFin" $ do
    ...
    it "var x; { fin x with x=1; fin with x=2; x=3 }" $ do
      remFin (Locals' ["x"] (Locals' [] (Seq (Fin (Just "x") (Write "x" (G.Const 1))) (Seq (Fin Nothing (Write "x" (G.Const 2))) (Write "x" (G.Const 3))))))
      `shouldBe` (Locals' ["x"] (Or (Fin' (Seq (Write "x" (G.Const 1)) Nop')) (Locals' [] (Or (Fin' (Write "x" (G.Const 2))) (Write "x" (G.Const 3))))))
    ...
    it "var x; { fin x with x=1; fin x with y=1; y=3 }" $ do
      ...
      `shouldBe` (Locals' ["x"] (Or (Fin' (Seq (Write "y" (G.Const 1)) (Seq (Write "x" (G.Const 1)) Nop'))) (Locals' [] (Write "y" (G.Const 3)))))
    ...
    it "var x; nop; { fin x with x=1; fin with x=2; x=3; fin x with y=1; fin with y=2; y=3 }; nop" $ do
      ...
      `shouldBe` (Locals' ["x"] (Or (Fin' (Seq (Write "y" (G.Const 1)) (Seq (Write "x" (G.Const 1)) Nop'))) (Seq Nop' (Seq (Locals' [] (Or (Fin' (Write "x" (G.Const 2))) (Seq (Write "x" (G.Const 3)) (Or (Fin' (Write "y" (G.Const 2))) (Write "y" (G.Const 3)))))) Nop'))))
  ...
  describe "toGrammar" $ do
    ...
    it "do var x; x = 1 end" $ do
      toGrammar (Locals (Seq (Var "x") (Write "x" (G.Const 1))))
      `shouldBe` G.Locals [] (G.Locals ["x"] (G.Seq G.Nop (G.Write "x" (G.Const 1))))

    it "spawn do await A; end ;; await B; var x; await FOREVER;" $ do
      toGrammar (Seq (Spawn (AwaitExt 0)) (Seq (AwaitExt 1) (Seq (Var "x") AwaitFor)))
      `shouldBe` (G.Locals ["x"] (G.Or (G.Seq (G.AwaitExt 0) (G.AwaitExt G.inputForever)) (G.Seq (G.AwaitExt 1) (G.Seq G.Nop (G.AwaitExt G.inputForever)))))


    it "spawn do async ret++ end ;; await F;" $ do
      toGrammar (Seq (Spawn (Async (Loop (Write "x" (G.Add (G.Read "x") (G.Const 1)))))) (AwaitExt 0))
      `shouldBe` (G.Locals [] (G.Or (G.Seq (G.Loop (G.Seq (G.Write "x" (G.Add (G.Read "x") (G.Const 1))) (G.AwaitExt (-3)))) (G.AwaitExt (-2))) (G.AwaitExt 0)))
